...
export function MaterialComparison({movements}:{movements:BusinessMovement[]}){
 ...
 return <section className="panel material-financial-chart"><div className="financial-chart-head"><div><h2>Ventas, costos y ganancias</h2><p>Selecciona una categoría y luego un tipo de residuo perteneciente a ella.</p></div><div className="chart-selectors"><label>Categoría<select value={category} onChange={e=>{setCategory(e.target.value);setMaterial('')}}><option value="">Todas las categorías</option>{categoryOptions.map(group=><option value={group.id} key={group.id}>{group.name}</option>)}</select></label><label>Tipo de residuo<select disabled={!category} value={material} onChange={e=>setMaterial(e.target.value)}><option value="">Todos los tipos de residuo</option>{materialOptions.map(name=><option key={name}>{name}</option>)}</select></label></div></div>
